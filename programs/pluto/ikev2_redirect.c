...
char *global_redirect_to = NULL;

static struct redirect_dests {
        char *whole;
        const char *next;
} global_dests;
...
void free_global_redirect_dests(void)
{
	...
	pfreeany(global_dests.whole);
}

static void init_redirect_dests(const char *rd_str, struct redirect_dests *dests)
{
	if (rd_str == NULL) {
		dests->whole = clone_str("", "redirect_dests");
	} else {
		/* strip any leading delimiters */
		const char *c = rd_str + strspn(rd_str, ", \t");
		dests->whole = clone_str(c, "redirect dests");
	}
	dests->next = dests->whole;
}

void init_global_redirect_dests(const char *grd_str)
{
	free_global_redirect_dests();
	init_redirect_dests(grd_str, &global_dests);
	/* global_redirect_to needed for pluto console message (plutomain.c) */
	global_redirect_to = clone_str(global_dests.whole, "global_redirect_to");
}
...
{
	ip_address ip_addr;
	char *dest_str = clone_bytes(dest.ptr, dest.len, "redirect dest_str");
	dest_str[dest.len] = '\0';
	err_t ugh = ttoaddr_num(dest_str, 0, AF_UNSPEC, &ip_addr);
	pfreeany(dest_str);
...
static bool allow_to_be_redirected(const char *allowed_targets_list, ip_address *dest_ip)
{
	...
	for (const char *t = allowed_targets_list;; ) {
		...
		if (ugh != NULL) {
			DBGF(DBG_CONTROLMORE, "address %.*s isn't a valid (%s)", len, t, ugh);
